...
class RoiHeadsSTA(RoiHeads):
    ...
    def postprocess_detections_sta(
        ...
        for boxes, scores, image_shape, verbs, ttcs in zip(pred_boxes_list, pred_scores_list, image_shapes, verb_predictions_list, ttc_predictions_list):
            ...
            inds = torch.where(scores > 0.0001)[0]
...
class RoiHeadsSTAv2(RoiHeadsSTA):
    ...
    def __init__(self, *args, fusion='sum', verb_topk=1, **kwargs):
        ...
        #aff_noun_np = np.load('/home/lmur/stillfast_baseline/stillfast/stillfast/nouns_gt_distribution.npy') / 100
        #self.aff_noun = F.softmax(torch.tensor(aff_noun_np, device='cuda').unsqueeze(0), dim = 1)
        #aff_verb_np = np.load('/home/lmur/stillfast_baseline/stillfast/stillfast/verbs_gt_distribution.npy') / 100
        #self.aff_verb = F.softmax(torch.tensor(aff_verb_np, device='cuda').unsqueeze(0), dim = 1)
        if fusion == 'sum':
    ...
    def forward(
        ...
        if self.fusion == 'sum':
            ...
            to_correct = False
            #print(box_features.shape, 'shape of the proposals')
    ...
    def postprocess_detections_sta(
        ...
        #aff_nouns = torch.tensor(self.aff_noun, device=device)
        #print(noun_logits.shape, verb_logits.shape, self.aff_noun.shape, self.aff_verb.shape)
        #aff_N_prior = self.aff_noun.repeat(noun_logits.shape[0], 1).to(noun_logits.device)
        #aff_V_prior = self.aff_verb.repeat(verb_logits.shape[0], 1).to(verb_logits.device)
        #print(aff_N_prior.shape)
        ...
        for boxes, noun_scores, image_shape, verb_scores, ttcs in zip(pred_boxes_list, noun_scores_list, image_shapes, verb_scores_list, ttc_predictions_list):
            ...
            ttcs = ttcs[keep_idx]
            #good_mask = (ttcs > 0.8) & (ttcs < 1.2)
            #vn_scores = vn_scores.unsqueeze(-1)
            #vn_scores = torch.where(good_mask, vn_scores * 5, vn_scores).squeeze(1)
            #ttcs_prior = torch.ones_like(ttcs) * 1.06
            #ttcs = (ttcs + ttcs_prior) / 2
