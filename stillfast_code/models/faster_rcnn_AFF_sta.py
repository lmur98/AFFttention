...
class RoiHeadsAFFSTAv2(RoiHeadsSTA):
    ...
    def __init__(self, *args, fusion='sum', verb_topk=1, **kwargs):
        ...
        print("RoiHeads with AFF v2-----------")
        self.fusion = fusion
        self.AFF_fusion = 'Bayes'
        print('The fusion of the affordances is ', self.AFF_fusion)
        ...
        if fusion == 'sum':
            ...
            self.mapper = nn.Sequential(
                #nn.Linear(256 + 1024, 1024), #MOD TO ADAPT DINO SIZE
                nn.Linear(384 + 1024, 1024),
                nn.ReLU(),
                nn.Linear(1024, 1024)
                )
    ...
    def forward(
        ...
        if self.training:
            proposed_boxes_per_img = 256
        else:
            proposed_boxes_per_img = 1000

        self._check_targets(targets)
        proposals, matched_idxs, noun_labels, regression_targets = self._select_training_samples(proposals, targets) #Proposals, list[(N, 4)] -> N is 256 (training) or 1000 (validation)        
        box_features = self._compute_box_features(features, proposals, image_shapes) # Tensor (N * BS, 1024)
        ...
        if self.fusion == 'sum':
            ...
            #-----Newww!!
            total_proposals = box_features.shape[0]
            to_correct = False

            if total_proposals != BS * proposed_boxes_per_img: 
                print('-----Waaarning', box_features.shape, self.training)   
                _box_features = torch.zeros((BS * proposed_boxes_per_img, 1024)).to(box_features.device)
                n_proposals = []
                for n in range(len(proposals)):
                    n_proposals.append(proposals[n].shape[0]) #List with the number of proposals per image
                
                start = 0
                for n in range(len(proposals)):
                    _start = n * proposed_boxes_per_img
                    _end = _start + n_proposals[n]
                    end = start + n_proposals[n]
                    _box_features[_start:_end] = box_features[start:end]
                    start = end
            ...
            #Now, we want to remove the zero offset that we introduced before
            if to_correct:
                for n in range(len(proposals)):
                    start = n * proposed_boxes_per_img
                    end = start + n_proposals[n]
                    if n == 0:
                        _gl_box_features = gl_box_features[start:end]
                    else:
                        _gl_box_features = torch.cat((_gl_box_features, gl_box_features[start:end]), dim=0)
                gl_box_features = _gl_box_features
        
        else:
            raise ValueError('Unknown fusion method: {}'.format(self.fusion))
        
        aff_fusion = 'final_logits'
        if aff_fusion == 'final_logits':
            aff_N_prior = torch.stack([t['AFF_N_GT'] for t in targets], dim=0).to(gl_box_features.device)
            aff_V_prior = torch.stack([t['AFF_V_GT'] for t in targets], dim=0).to(gl_box_features.device)
            
            aff_N_prior = aff_N_prior.unsqueeze(1).expand(BS, proposed_boxes_per_img, aff_N_prior.shape[-1]) #(BS, ch) -> (BS, num_props, ch)
            aff_N_prior = aff_N_prior.reshape(-1, aff_N_prior.shape[-1])
            aff_V_prior = aff_V_prior.unsqueeze(1).expand(BS, proposed_boxes_per_img, aff_V_prior.shape[-1])
            aff_V_prior = aff_V_prior.reshape(-1, aff_V_prior.shape[-1])


            if to_correct:
                print('correcting the aff priors')
                for n in range(len(proposals)):
                    start = n * proposed_boxes_per_img
                    end = start + n_proposals[n]
                    if n == 0:
                        _aff_N_prior = aff_N_prior[start:end]
                        _aff_V_prior = aff_V_prior[start:end]
                    else:
                        _aff_N_prior = torch.cat((_aff_N_prior, aff_N_prior[start:end]), dim=0)
                        _aff_V_prior = torch.cat((_aff_V_prior, aff_V_prior[start:end]), dim=0)
                aff_N_prior = _aff_N_prior
                aff_V_prior = _aff_V_prior
                print(gl_box_features.shape, aff_N_prior.shape, aff_V_prior.shape)

            class_logits, box_regression = self.box_predictor(gl_box_features) # + aff_N_prior)
            verb_logits = self.verb_predictor(gl_box_features) # + aff_V_prior)
        else:
    ...
    def postprocess_detections_sta(
        ...
        if self.AFF_fusion == 'Bayes':
            # apply softmax to get noun and verb scores
            noun_scores_likelihood = F.softmax(noun_logits, -1)
            verb_scores_likelihood = F.softmax(verb_logits, -1)

            #Bayes theorem
            noun_scores = aff_N_prior * noun_scores_likelihood
            verb_scores = aff_V_prior * verb_scores_likelihood
            print('including aff')
            noun_scores = noun_scores / noun_scores.sum(-1, keepdim=True)
            verb_scores = verb_scores / verb_scores.sum(-1, keepdim=True)
        ...
        for boxes, noun_scores, image_shape, verb_scores, ttcs in zip(pred_boxes_list, noun_scores_list, image_shapes, verb_scores_list, ttc_predictions_list):
            ...
            # remove low scoring boxes
            #self.score_thresh
            inds = torch.where(vn_scores > 0.002)[0] #self.score_thresh
