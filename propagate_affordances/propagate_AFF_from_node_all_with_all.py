...
class Aff_propagator():
    def __init__(self, dataset):
        ...
        self.visual_EgoVLP_train_feats = '/home/lmur/catania_lorenzo/v2_data_extracted/EgoVLP_train_feats/videos_EgoVLP_resize224'
        self.text_EgoVLP_train_feats = '/home/lmur/catania_lorenzo/v2_data_extracted/EgoVLP_train_feats/narrations_EgoVLP'
        self.train_root = '/home/lmur/catania_lorenzo/v2_data_extracted/output_topo_graphs_TRAIN/local_topological_graphs_REVIEWED'
        #self.from_multiple_topo_graphs_to_all_nodes(self.train_root)
        self.train_nodes = json.load(open('/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/only_nodes.json', 'r'))
        self.still_frames_path = '/home/furnari/data/ego4d/v2-15-02-23/object_frames/'

        # Val dataset: visual descriptions features
        self.val_EgoVLP_feats = '/home/lmur/catania_lorenzo/v2_data_extracted/EgoVLP_val_feats/videos_EgoVLP_resize224'

        #self.load_training_clusters()
        #self.load_validation_queries()
        self.V_val_desc = torch.load('/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/V_validation_desc.pt', map_location=torch.device('cuda:3'))
        self.V_training_node_mean = torch.load('/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/V_training_desc.pt', map_location=torch.device('cuda:3')).squeeze()
        self.T_training_node_mean = torch.load('/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/T_training_desc.pt', map_location=torch.device('cuda:3')).squeeze()
        print('The shape of the training nodes is:', self.V_training_node_mean.shape)
        print('The shape of the training nodes is:', self.T_training_node_mean.shape)
        print('The shape of the validation nodes is:', self.V_val_desc.shape)
    ...
    def from_multiple_topo_graphs_to_all_nodes(self, topological_graphs_dir):
        ...
        for v_id in tqdm.tqdm(self.total_vids):
            ...
            for node in local_graph:
                ...
                all_node_data.append({'v_id': v_id, 
                                      ...
                                      'STA': node['STA'],})
                all_text += len(node['narrator_1'])
                all_visits += len(node['frames_in_node'])
                all_sta += len(node['STA'])
        #Save the list of dictionaries
        with open('/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/only_nodes.json', 'w') as f:
    ...
    def load_training_clusters(self):
        ...
        for node in tqdm.tqdm(self.train_nodes):
            ...
            for f in node['visits']:
                V_node.append(self.load_single_frame_EgoVLP(self.visual_EgoVLP_train_feats, node['v_id'], f).unsqueeze(0))
                T_node.append(self.load_single_frame_EgoVLP(self.text_EgoVLP_train_feats, node['v_id'], f).unsqueeze(0))
            total_frames += len(node['visits'])
        ...
        torch.save(self.V_training_node_mean, '/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/V_training_desc.pt')
        torch.save(self.T_training_node_mean, '/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/T_training_desc.pt')
    ...
    def load_validation_queries(self):
        ...
        for f in tqdm.tqdm(self.dataset):
            v_desc = self.load_single_frame_EgoVLP(self.val_EgoVLP_feats, f['v_id'], f['frame']).unsqueeze(0)
            self.V_val_desc.append(v_desc)
        self.V_val_desc = torch.cat(self.V_val_desc, dim=0)
        torch.save(self.V_val_desc, '/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/V_validation_desc.pt')
    ...
    def from_STA_node_to_AFF_vector(self, STA_list):
        ...
        for f, sta in enumerate(STA_list):
            ...
            STA_interaction = STA_interaction.replace('tape_measure', 'measure')
            STA_interaction = STA_interaction.replace('rubber_band', 'rubber')
            STA_interaction = STA_interaction.replace('chopping_board', 'chopping')
            STA_interaction = STA_interaction.replace('nail_polish', 'nail')
            STA_interaction = STA_interaction.replace('puzzle_or_game_piece', 'puzzle')
            STA_interaction = STA_interaction.replace('remote_control', 'remote')
            STA_interaction = STA_interaction.replace('sewing_machine', 'sewing')
            STA_interaction = STA_interaction.replace('solder_iron', 'solder')
            STA_interaction = STA_interaction.replace('vacuum_cleaner', 'vacuum')
            STA_interaction = STA_interaction.replace('weighing_scale', 'weighing')
    ...
    def visualize_closest_nodes(self, closest_V, closest_T, v_id, frame, sta_noun, sta_verb, AFF_nouns, AFF_verbs, base_N, base_V):
        #Aff plot graphs
        AFF_N_idx = np.argsort(AFF_nouns)[-20:] - 1 #Remove the background
        AFF_V_idx = np.argsort(AFF_verbs)[-20:] - 1
        ...
        for bar, prob in zip(barras_n, AFF_N_probs):
            if prob <= base_N:
                bar.set_color('blue')
            else:
                bar.set_color('green')
        ...
        for bar, prob in zip(barras_v, AFF_V_probs):
            if prob <= base_V:
                bar.set_color('blue')
            else:
                bar.set_color('green')
        ...
        plt.show()
        #Save the figure
        out_dir = '/home/lmur/catania_lorenzo/v2_data_extracted/qualitative_aff_examples'
...
N_acc = {'top1': 0, 'top3': 0, 'top5': 0, 'top7': 0}
V_acc = {'top1': 0, 'top3': 0, 'top5': 0, 'top7': 0}
number_AFF_N = {'top1': 0, 'top3': 0, 'top5': 0, 'top7': 0}
number_AFF_V = {'top1': 0, 'top3': 0, 'top5': 0, 'top7': 0}
STA_prob_N = {'top1': [], 'top3': [], 'top5': [], 'top7': []}
STA_prob_V = {'top1': [],'top3': [], 'top5': [], 'top7': []}
n_samples = 1
...
for f, frame in enumerate(tqdm.tqdm(val_dataset)):
    ...
    print('The shape of the visual dist is:', visual_dist_nodes.shape)
    print('The shape of the cross dist is:', cross_dist_nodes.shape)
    
    #Select the cluster where there is the frame with the lowest distance
    output = {'v_id': frame['v_id'], 'frame': frame['frame']}

    for k in N_acc.keys():
        joined_dist = np.concatenate((visual_dist_nodes, cross_dist_nodes), axis=0)
        topk = np.argsort(joined_dist)[:int(k[3:]) * 2]
        joined_dist = joined_dist[topk]
        
        topk_visual = topk[topk < len(visual_dist_nodes)]
        topk_cross = topk[topk >= len(visual_dist_nodes)]- len(visual_dist_nodes)
        visual_dist = visual_dist_nodes[topk_visual]
        cross_dist = cross_dist_nodes[topk_cross]
        
        if k != 'top1':
            if len(visual_dist) > 0:
                min_V = np.min(visual_dist)
                topk_visual = topk_visual[visual_dist < 2.00 * min_V]
                visual_dist = visual_dist[visual_dist < 2.00 * min_V]
            if len(cross_dist) > 0:
                min_T = np.min(cross_dist)
                topk_cross = topk_cross[cross_dist < 2.00 * min_T]
                cross_dist = cross_dist[cross_dist < 2.00 * min_T]
        ...
        if len(topk_visual) > 0 and len(topk_cross) > 0:
            joined_dist = torch.cat((torch.tensor(visual_dist), torch.tensor(cross_dist)))
        elif len(topk_visual) > 0:
            joined_dist = torch.tensor(visual_dist)
        elif len(topk_cross) > 0:
            joined_dist = torch.tensor(cross_dist)
...
out_dir = '/home/lmur/catania_lorenzo/v2_data_extracted/AFF_results'
if not os.path.exists(out_dir):
    os.makedirs(out_dir)

#Save the list as a numpy
np.save(os.path.join(out_dir, 'predicted_AFF_only_nodes_weighted_MAX_normalized_softmaxT2_text_visual_all_with_all_nofilter.npy'), predicted_AFF)
