...
class Aff_propagator():
    def __init__(self, dataset):
        ...
        self.V_val_desc = torch.load('/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/V_val_desc.pt', map_location=torch.device(self.device))
        self.V_training_node_mean = torch.load('/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/V_training_desc.pt', map_location=torch.device(self.device)).squeeze()
        self.T_training_node_mean = torch.load('/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/T_training_desc.pt', map_location=torch.device(self.device)).squeeze()
    ...
    def load_validation_queries(self):
        ...
        torch.save(self.V_val_desc, '/home/lmur/catania_lorenzo/v2_data_extracted/mean_descriptor_per_node/V_val_desc.pt')
    ...
    def load_single_frame_EgoVLP(self, root, v_id, f):
        EgoVLP_video_feats = torch.load(os.path.join(root, f"{v_id}_{f:07d}.pt"), map_location=torch.device(self.device))
        EgoVLP_video_feats /= EgoVLP_video_feats.norm(dim=-1, keepdim=True)
        return EgoVLP_video_feats.squeeze().to(self.device)
    ...
    def from_STA_node_to_AFF_vector(self, STA_list):
        ...
        for f, sta in enumerate(STA_list):
            ...
            if len(noun_id) > 1:
                if sta_verb == 'tighten' or sta_verb == 'attach' or sta_verb == 'screw' or sta_verb == 'loosen':
                    noun_id = [46]
                else:
                    noun_id = [101]
    ...
    def visualize_closest_nodes(self, closest_V, closest_T, v_id, frame, sta_noun, sta_verb, AFF_nouns, AFF_verbs, base_N, base_V):
        ...
        plt.show()
        #Save the figure
        out_dir = '/home/lmur/catania_lorenzo/data_extracted/AFF_final_Ego4D_v2'
...
number_AFF_N = {'top1': 0, 'top3': 0, 'top5': 0}
number_AFF_V = {'top1': 0, 'top3': 0, 'top5': 0}
STA_prob_N = {'top1': [], 'top3': [], 'top5': []}
STA_prob_V = {'top1': [],'top3': [], 'top5': []}
n_samples = 1
...
for f, frame in enumerate(tqdm.tqdm(val_dataset)):
    ...
    for k in N_acc.keys():
        #topk = np.argsort([visual_dist_nodes, cross_dist_nodes])[:int(k[3:]) *]
        print('Visual dist nodes', visual_dist_nodes)
        ...
        AFF_nouns = softmax(torch.tensor(AFF_nouns) * 1).numpy()
        AFF_verbs = softmax(torch.tensor(AFF_verbs) * 1).numpy()
...
#np.save(os.path.join(out_dir, 'AFF_v2_T1.npy'), predicted_AFF)
